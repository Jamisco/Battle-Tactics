...
/// <summary>
        ...
        public Collider GetBaseMeshCollider()
        {
            MeshLayer layer = sortedChunks.First().Value.GetMeshLayer(baseLayerId);

           return layer.GetMeshCollider();
        }
...
/// <summary>
        ...
        public void SortMeshLayers()
        {
            ...
            // this will determine if we offset by adding or subtracting.
            // Sometimes the camera may be looking at the camera from a different angle, thus we may need to negate the offset
            bool negate = false;
                            ...
                            string layerId = USE_DEFAULT_LAYER)
        {
            ...
            if(data == null)
            {
                Debug.LogError("Visual Data cannot be null");
                return;
            }
        ...
        /// <summary>
        /// The multithreading version of the InsertPostionBlock method. This method is faster than the non-multithreading version. However, may or may not work (most likely will) on some computers. Inorder to maximize performance, there is no error checking. Make sure your data is valid before calling this method
        /// </summary>
        ...
        /// <summary>
        /// Will insert a block of visual data at the given positions. If the number of positions and datas are not the same, the method will abort. Inorder to maximize performance, there is no error checking. Make sure your data is valid before calling this method
        /// </summary>
        /// <param name="positions"></param>
        /// <param name="datas"></param>
        /// <param name="layerId"></param>
        ...
        public void Clear()
        {
            ...
            foreach (GridChunk gridChunk in transform.GetComponentsInChildren<GridChunk>())
            {
                gridChunk.Clear();
            }
